...
import { X, ChevronLeft, ChevronRight, Sparkles, Volume2, Shield, Zap } from 'lucide-react';
...
export default function Card3DModal({ card, onClose, onNext, onPrev }) {
  ...
  // Track mouse movement to calculate 3D tilt angles
  const handleMouseMove = (e) => {
    ...
    // Calculate max tilt angle (+- 25 degrees)
    ...
    // Calculate Holographic Sheen Gradient Position
    const posX = ((e.clientX - rect.left) / rect.width) * 100;
    const posY = ((e.clientY - rect.top) / rect.height) * 100;
    setSheenPos({ x: posX, y: posY });

    // Occasional subtle sheen sound
    if (Math.abs(mouseX) % 40 < 5) {
      playLaserSheenSound();
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fadeIn">
      ...
      <div className="relative w-full max-w-5xl bg-slate-900/90 border border-cyan-500/40 rounded-2xl p-6 md:p-8 shadow-2xl overflow-hidden flex flex-col md:flex-row gap-8 items-center">
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full modal-3d-perspective py-6">
          <div className="text-cyan-300/80 text-xs tracking-widest uppercase mb-3 flex items-center gap-1 font-mono">
            <Sparkles className="w-3.5 h-3.5 text-cyan-400 animate-spin" />
            滑鼠懸停／移動可多角度探索 3D 立體繪畫與鐳射卡面
          </div>
          ...
          <div
            ...
            className={`relative w-72 md:w-80 card-3d-wrapper ygo-card-container ${frameClass} cursor-grab active:cursor-grabbing shadow-2xl border-4 border-yellow-500/80`}
          >
            ...
            <div className="ygo-inner-frame h-full flex flex-col justify-between">
              ...
              <div className="ygo-title-bar">
                <span className="ygo-card-name text-base">{card.name}</span>
              ...
              {/* Card Description */}
              <div className="ygo-desc-box text-xs">
                ...
                {isMonster && (
                  <div className="ygo-stats-bar text-xs">
          ...
          <div className="flex gap-4 mt-6">
            <button
              onClick={onPrev}
              className="flex items-center gap-1 px-4 py-2 bg-slate-800/80 hover:bg-cyan-900/60 border border-slate-700 hover:border-cyan-500/50 rounded-lg text-sm text-cyan-200 transition"
            >
              <ChevronLeft className="w-4 h-4" /> 上一張卡牌
            </button>
            <button
              onClick={onNext}
              className="flex items-center gap-1 px-4 py-2 bg-slate-800/80 hover:bg-cyan-900/60 border border-slate-700 hover:border-cyan-500/50 rounded-lg text-sm text-cyan-200 transition"
            >
              下一張卡牌 <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        </div>

        {/* Card Details Info Panel (Right Side) */}
        <div className="w-full md:w-96 flex flex-col gap-4 bg-slate-950/60 p-6 rounded-xl border border-cyan-500/30">
          <div>
            <span className="inline-block px-2.5 py-1 text-xs font-semibold rounded bg-cyan-950 text-cyan-300 border border-cyan-700/50 mb-2">
              {card.rarity}
            </span>
            <h2 className="text-2xl font-black text-white tracking-wide">{card.name}</h2>
            <p className="text-xs text-cyan-400/80 font-mono mt-0.5">{card.japaneseName} • {card.englishName}</p>
          </div>

          {/* Quote */}
          {card.quote && (
            <div className="italic text-xs text-cyan-200/90 bg-cyan-950/40 p-3 rounded-lg border-l-2 border-cyan-400">
              {card.quote}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="bg-slate-900/80 p-2.5 rounded border border-slate-800">
              <span className="text-slate-400 block">卡片種類</span>
              <span className="text-white font-medium capitalize">
                {isSynchro ? '白色卡面 (同步怪獸)' : isMonster ? '效果怪獸' : isSpell ? '魔法卡' : '陷阱卡'}
              </span>
            </div>
            <div className="bg-slate-900/80 p-2.5 rounded border border-slate-800">
              <span className="text-slate-400 block">屬性 / 種族</span>
            ...
            {isMonster && (
              <>
                <div className="bg-slate-900/80 p-2.5 rounded border border-slate-800">
                  <span className="text-slate-400 block">攻擊力 (ATK)</span>
                  <span className="text-yellow-400 font-bold text-sm">{card.atk}</span>
                </div>
                <div className="bg-slate-900/80 p-2.5 rounded border border-slate-800">
                  <span className="text-slate-400 block">守備力 (DEF)</span>
          ...
          <div className="mt-2">
            ...
            <div className="text-xs text-slate-300 leading-relaxed bg-slate-900 p-3.5 rounded-lg border border-slate-800 max-h-48 overflow-y-auto whitespace-pre-line">
              {card.description}
            </div>
          </div>
        </div>
